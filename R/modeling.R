...
run_aen_peaks_for_gene <- function(input_list,
    ...
    weights = 1 / (abs(gcdnet::coef(aen.fit)[-1, ])^gamma + 1/num_pseudo_cells)
    ...
    for (i in 1:num_bootstraps) {
        ...
        lambda_to_use = ifelse(lambda == "lambda.min", aen.fit$lambda.min, aen.fit$lambda.1se)
 		...
 		if (recalculate_l1) {
        	...
        	weights = 1 / (gcdnet::abs(coef(aen.fit)[-1, ])^gamma + 1/num_pseudo_cells) 
        }
...
#' Run adaptive elastic net to identify TFs regulating target genes with paired, non-multiome single cell datasets
#'
#' This function will identify putative transcription factors regulating each of
...
#' @param seurat A Seurat object
#' @param pseudocell_mat Pseudocell expression matrix to use
#' @param motif_matrix_df motif-peak dataframe returned from running make_motif_matrix on a scATAC-seq Seurat object and a PWM list of motifs to use
#' @param peak_gene_key peak-gene dataframe returned from running conns_to_peak_gene_key on the connections generated by Cicero
#' @param gene_list Names of genes to model
#' @param lambda2 Parameter for quadratic L2 penalty for regularization
#' @param gamma Positive constant for calculating adaptive weights
#' @param ci_cutoff Confidence interval cutoff used to determine whether an estimated non-zero coefficient should be kept
...
#' @export
#'
run_tf_aen_ccan <- function(seurat,
                       pseudocell_mat,
                       motif_matrix_df,
                       peak_gene_key,
                       gene_list = NULL,
	...
	motif_matrix_df <- motif_matrix_df[, which(colnames(motif_matrix_df) %in% colnames(pseudocell_mat))]
	tf_names <- unique(colnames(motif_matrix_df))

	peak_gene_key <- peak_gene_key[which(peak_gene_key$gene %in% modeled_genes), ]

	all_linked_genes <- unique(peak_gene_key$gene)
	linked_peak_list <- lapply(all_linked_genes, function(x) {
	                        df <- peak_gene_key[which(peak_gene_key$gene == x), ]
	                        return(unique(df$peak))
	                    })
	names(linked_peak_list) <- all_linked_genes

	tf_aggregates <- lapply(linked_peak_list, peaks_to_tfs_ccan, key_tibble = motif_matrix_df)
	names(tf_aggregates) <- all_linked_genes
	tf_aggregates <- tf_aggregates[which(unlist(lapply(tf_aggregates, length)) > 1)]
...
#' Utility function used to generate peak-gene key dataframe from the output of Cicero or an equivalent method
#'
#' This function takes in a conns dataframe generated by Cicero or an equivalent method and 
#' a single cell ATAC-seq Seurat object. It generates CCANs then uses the Annotation
#' stored in the Seurat object to identify CCANs with peaks that overlap with a gene's position
#' to link putative cis-regulatory elements to each gene.
#' 
#' @param conns Connection dataframe generated by Cicero or equivalent method
#' @param seurat_atac Single cell ATAC-seq Seurat object with stored Annotation
#' @param mc.cores Number of threads to use for multithreading
#'
#' @return Peak-gene dataframe for use by run_tf_aen_ccan
#' @export
#'
conns_to_peak_gene_key <- function(conns, seurat_atac, mc.cores = 8) {
	require(Signac)
	require(GenomicRanges)
	require(cicero)
	require(stringr)
	...
	gtf = Signac:::CollapseToLongestTranscript(Annotation(atac))
	...
	gtf.body_prom <- Signac:::Extend(x = gtf, upstream = 2000, downstream = 0)

	peak.df <- as.data.frame(str_split(rownames(atac), pattern = "-", simplify = TRUE))
	colnames(peak.df) <- c("chr", "start", "end")
	peak.gr <- GenomicRanges::makeGRangesFromDataFrame(peak.df)
	gene.distances <- GenomicRanges::distanceToNearest(x = peak.gr, subject = gtf.body_prom, select = "all")
	...
	gene.ids <- gtf.body_prom[S4Vectors::subjectHits(x = keep.overlaps)]
	peak.ids$gene.name <- gene.ids$gene_name
	peak.ids <- as.data.frame(x = peak.ids, row.names = NULL)

	peak.ids$peak <- paste0(peak.ids$seqnames, "-", peak.ids$start, "-", peak.ids$end)
	annotations <- peak.ids[, c("peak", "gene.name")]

	ccans <- generate_ccans(conns)
	ccan_annotations <- mclapply(unique(annotations$gene.name), function(x) {
	        gene_peaks <- annotations[which(annotations$gene.name == x), "peak"]
	        containing_ccans <- ccans[annotations[which(annotations$gene.name == x), "peak"], ]$CCAN %>% unique
	        containing_ccans <- which(!is.na(containing_ccans))
	        ccan_peaks <- ccans[which(ccans$CCAN %in% containing_ccans), "Peak"] %>% unique
	        return(data.frame(peak = unique(c(gene_peaks, ccan_peaks)), gene = x))
	    }, mc.cores = 8)
	peak_gene_key <- bind_rows(ccan_annotations)

	return(peak_gene_key)
}

#' Utility function used to generate 
#'
#' This function takes in a single cell ATAC-seq Seurat object and a Position Weight Matrix list.
#' It generates a peak by TF dataframe through Signac's AddMotifs function.
#'
#' @param seurat_atac Single cell ATAC-seq Seurat object with stored Annotation
#' @param pwm_list PWMList to use. Default is human_pwms_v2 from chromVARmotifs
#' @param ref_genome BSgenome for AddMotifs to use
#'
#' @return Peak-TF dataframe for use by run_tf_aen_ccan
#' @export
#'
make_motif_matrix <- function(seurat_atac, pwm_list, ref_genome = BSgenome.Hsapiens.UCSC.hg38) {
	require(Signac)
	...
	motif_matrix <- AddMotifs(seurat_atac, genome = ref_genome, pfm = pwm_list)
	motif_matrix_df <- motif_matrix@assays$peaks@motifs@data
	colnames(motif_matrix_df) <- name(pwm_list)

	return(motif_matrix_df)
}

#' Utility function used to convert a peak list for a given gene to a list of TFs with predicted motifs within the peak list, used in run_tf_aen_ccan
#'
#' This function takes in a peak list and peak-TF key as input and
#' returns the TF names with at least one motif present in the 
#' peak list, as determined by the peak-TF key.
...
#' @export
#'
peaks_to_tfs_ccan <- function(peak_list, key_tibble) {
    require(tidyverse)
    
    if (length(peak_list) == 1 && peak_list == "") {
        return("")
    } else if (length(peak_list) == 1) {
        subset_tibble <- key_tibble[peak_list, ]
        subset <- names(subset_tibble)[which(subset_tibble > 0)] %>% unique
        return(subset)
    }
    subset_tibble <- key_tibble[peak_list, ]
    subset <- colSums(subset_tibble)
    subset <- names(subset)[which(subset > 0)] %>% unique
    return(subset)
}

#' Utility function used to convert list of lists output of run_tf_aen to more useful dataframe format
#' 
#' This function converts the output of run_tf_aen to a TF by gene dataframe.
#' Each position is the predicted regulatory weight of the given TF for the 
#' given gene's expression.
#' 
#' @param aen_results Modeling output of run_tf_aen
#'
#' @return TF-gene dataframe containing modeling results, with rows representing each modeled gene.
#' @export
#'
make_tf_model_df <- function(aen_results) {
    require(tidyverse)

    tf_model <- lapply(aen_results, function(x) x[[4]][union(1, which(x[[4]][, "coef_if_kept"] != 0)), "coef_if_kept"])
    tf_model <- tf_model[which(unlist(lapply(tf_model, length)) > 1)]
    gene_names <- names(tf_model)
    tf_model <- bind_rows(tf_model)
    tf_model[is.na(tf_model)] <- 0
    tf_model <- tf_model %>% as.data.frame %>% as.matrix
    rownames(tf_model) <- gene_names
    return(tf_model)
}

#' Utility function used to convert list of lists output of run_peak_aen to more useful dataframe format
#' 
#' This function converts the output of run_peak_aen to a CRE by gene dataframe.
#' Each position is the predicted regulatory weight of the given CRE for the 
#' given gene's expression.
#' 
#' @param aen_results_peaks Modeling output of run_peak_aen
#'
#' @return CRE-gene dataframe containing modeling results, with rows representing each modeled gene.
#' @export
#'
make_cre_model_df <- function(aen_results_peaks) {
    require(tidyverse)

    cre_model <- lapply(aen_results_peaks, function(x) x[[4]][union(1, which(x[[4]][, "coef_if_kept"] != 0)), "coef_if_kept"])
    names(cre_model) <- names(aen_results_peaks)
    cre_model <- cre_model[which(unlist(lapply(cre_model, length)) > 1)]
    gene_names <- names(cre_model)
    cre_model <- bind_rows(cre_model)
    cre_model[is.na(cre_model)] <- 0
    cre_model <- cre_model %>% as.data.frame %>% as.matrix
    rownames(cre_model) <- gene_names
    return(cre_model)
}
